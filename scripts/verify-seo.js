...
assertFile(sitemapPath, "sitemap.xml");
assert.ok(!fs.existsSync(path.join(siteDir, "sitemap.xml", "index.html")), "sitemap must not be written as sitemap.xml/index.html");
assertFile(redirectsPath, "_redirects");
const redirects = fs.readFileSync(redirectsPath, "utf8");
assert.ok(
  /\/sitemap\.xml\s+\/sitemap\.xml\/\s+301!/.test(redirects),
  "_redirects must 301 /sitemap.xml to /sitemap.xml/"
);
...
console.log("SEO artifacts OK: _site/sitemap.xml, _site/_redirects, and _site/robots.txt");
